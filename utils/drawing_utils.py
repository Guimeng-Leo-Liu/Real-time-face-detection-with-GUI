...
from cv2 import arrowedLine, circle, rectangle, line
import dataclasses
...
def _normalized_to_pixel_coordinates(
  ...
  if not (is_valid_normalized_value(normalized_x) and
      ...
      x_px = max(math.floor(normalized_x * image_width), 0)
      y_px = max(math.floor(normalized_y * image_height), 0)
  else:
...
def draw_detection(
  ...
  # Draws keypoints.
  landmark = []
  for keypoint in location.relative_keypoints: # 右眼左眼鼻嘴右耳左耳
    keypoint_px = _normalized_to_pixel_coordinates(keypoint.x, keypoint.y,
                                                   image_cols, image_rows)
    # print(keypoint_px)
    landmark.append(keypoint_px)
    circle(image, keypoint_px, keypoint_drawing_spec.circle_radius,
  ...
  rect_end_point = _normalized_to_pixel_coordinates(
      ...
      image_rows)
  rectangle(image, rect_start_point, rect_end_point,
...
def draw_landmarks(
  ...
  if connections:
    ...
    for connection in connections:
      ...
      if start_idx in idx_to_coordinates and end_idx in idx_to_coordinates:
        ...
        line(image, idx_to_coordinates[start_idx],
  ...
  if landmark_drawing_spec:
    for idx, landmark_px in idx_to_coordinates.items():
      ...
      circle(image, landmark_px, circle_border_radius, WHITE_COLOR,
                 drawing_spec.thickness)
      # Fill color into the circle
      circle(image, landmark_px, drawing_spec.circle_radius,
...
def draw_axis(
  ...
  arrowedLine(image, origin, x_axis, RED_COLOR, axis_drawing_spec.thickness)
  arrowedLine(image, origin, y_axis, GREEN_COLOR,
                  axis_drawing_spec.thickness)
  arrowedLine(image, origin, z_axis, BLUE_COLOR,
                  axis_drawing_spec.thickness)
